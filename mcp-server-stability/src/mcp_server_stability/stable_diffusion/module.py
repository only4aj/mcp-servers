...
class StabilityService:
    ...
    async def send_generation_request(self, params: dict) -> httpx.Response:
        ...
        try:
            ...
            raise StableDiffusionServerConnectionError(f"Error during request: {exc}") from exc

        if not response.is_success:
            raise StableDiffusionClientError(f"API error: {response.status_code} - {response.text}")
