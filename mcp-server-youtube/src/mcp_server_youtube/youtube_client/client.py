...
from mcp_server_youtube.youtube_client.models import (
    YouTubeVideo,
)
...
class YouTubeSearcher:
    ...
    def search_videos(self, query: str, max_results: int = 15, language: str = 'en') -> list[YouTubeVideo]:
        """
        ...
        try:
            ...
            try:
                ...
                error_message = error_details.get('error', {}).get('message', error_content)
            ...
            raise YouTubeApiError(msg, status_code=status_code, details=error_details) from e 
    ...
    def _get_transcript_by_id(self, video_id: str, language: str) -> str:
        ...
        """
        logger.debug(f"Attempting to fetch transcript for video ID: {video_id} in language: {language}")
        try:
            ...
            raise YouTubeTranscriptError(video_id=video_id, message=msg) from e
             ...
             msg = f"No transcript found for video ID {video_id} in language '{language}'."
             logger.warning(msg)
             raise YouTubeTranscriptError(video_id=video_id, message=msg) from e
