...
from mcp_server_twitter.logging_config import (configure_logging,
                                                  logging_level)
from mcp_server_twitter.server import server
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--host",
        default=os.getenv("MCP_TWITTER_HOST", "0.0.0.0"),  # Override with your env variables
        help="Host to bind to (Default: MCP_TWITTER_HOST or 0.0.0.0)", # Override with your env variables
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("MCP_TWITTER_PORT", "8000")), # Override with your env variables
        help="Port to listen on (Default: MCP_TWITTER_PORT or 8000)", # Override with your env variables
    )
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    uvicorn.run(
        "mcp_server_twitter.__main__:create_starlette_app",
        host=args.host,
        port=args.port,
        reload=args.reload,
